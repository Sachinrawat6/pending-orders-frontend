...
const ProcessedOrdersPage = () => {
  const { processed, stockInfoByStyle, loading, error, reload } = useOrdersOverview();
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500">Orders that have been marked processed.</p>
        </div>
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-3 py-1 text-sm font-medium text-emerald-700 ring-1 ring-inset ring-emerald-200">
            <span className="font-display font-semibold">{processed.length}</span> processed
          </span>
          <button
            type="button"
            onClick={reload}
      ...
      {loading && (
        <div className="flex items-center justify-center gap-2 py-16 text-slate-500">
          ...
          <span className="text-sm">Loading processed orders…</span>
        </div>
      )}
      ...
      {!loading && filtered.length === 0 && (
        <EmptyState
          icon={FiCheckCircle}
          title={search ? 'No matches' : 'Nothing processed yet'}
          description={
            search
              ? 'No processed order matches that search.'
              : 'Orders marked processed from Ready for Process will appear here.'
          }
        />
      )}
